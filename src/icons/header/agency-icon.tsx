import React from "react";
import { SvgIcon } from "@mui/material";

const AgencyIcon = () => {
  return (
    <SvgIcon style={{ width: 20, height: 20 }} viewBox="0 0 20 20">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="20"
        height="20"
        viewBox="0 0 20 20"
        fill="none"
      >
        <g clipPath="url(#clip0_12242_122610)">
          <path
            d="M5 18.3337V3.33366C5 2.89163 5.17559 2.46771 5.48816 2.15515C5.80072 1.84259 6.22464 1.66699 6.66667 1.66699H13.3333C13.7754 1.66699 14.1993 1.84259 14.5118 2.15515C14.8244 2.46771 15 2.89163 15 3.33366V18.3337H5Z"
            stroke="currentColor"
            strokeWidth="2.004"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M5.0013 10H3.33464C2.89261 10 2.46868 10.1756 2.15612 10.4882C1.84356 10.8007 1.66797 11.2246 1.66797 11.6667V16.6667C1.66797 17.1087 1.84356 17.5326 2.15612 17.8452C2.46868 18.1577 2.89261 18.3333 3.33464 18.3333H5.0013"
            stroke="currentColor"
            strokeWidth="2.004"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M15 7.5H16.6667C17.1087 7.5 17.5326 7.6756 17.8452 7.98816C18.1577 8.30072 18.3333 8.72464 18.3333 9.16667V16.6667C18.3333 17.1087 18.1577 17.5326 17.8452 17.8452C17.5326 18.1577 17.1087 18.3333 16.6667 18.3333H15"
            stroke="currentColor"
            strokeWidth="2.004"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M8.33203 5H11.6654"
            stroke="currentColor"
            strokeWidth="2.004"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M8.33203 8.33301H11.6654"
            stroke="currentColor"
            strokeWidth="2.004"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M8.33203 11.667H11.6654"
            stroke="currentColor"
            strokeWidth="2.004"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M8.33203 15H11.6654"
            stroke="currentColor"
            strokeWidth="2.004"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </g>
        <defs>
          <clipPath id="clip0_12242_122610">
            <rect width="20" height="20" fill="white" />
          </clipPath>
        </defs>
      </svg>
    </SvgIcon>
  );
};

export default AgencyIcon;
